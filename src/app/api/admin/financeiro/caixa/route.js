...
export async function GET(request) {
  ...
  try {
    ...
    const totalResult = await sequelize.query(
      `
      SELECT
        COALESCE(SUM(valor) FILTER (WHERE direcao = 'entrada'), 0) AS total_entradas_geral,
        COALESCE(SUM(valor) FILTER (WHERE direcao = 'saida'), 0) AS total_saidas_geral
      FROM caixa_plataforma
      WHERE DATE(criado_em) >= :de
        AND DATE(criado_em) <= :ate
      `,
    ...
    const saldoCaixa = (
      parseFloat(totalRow.total_entradas_geral || 0) -
      parseFloat(totalRow.total_saidas_geral || 0)
    ).toFixed(2);
    ...
    return NextResponse.json({
      ...
      resumo: {
        total_entradas: parseFloat(totalRow.total_entradas_geral || 0).toFixed(2),
        total_saidas: parseFloat(totalRow.total_saidas_geral || 0).toFixed(2),
    ...
    console.error(`[Admin Financeiro] Erro ao buscar dados de caixa: ${error.message}`);
    return NextResponse.json(
      { error: "Erro ao buscar dados de caixa da plataforma." },
      { status: 500 }
    );
  }
}
